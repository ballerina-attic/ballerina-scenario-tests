...
readonly test_cb_with_retry_parent_path=$( cd "$(dirname "${BASH_SOURCE[0]}")" ; pwd -P )
readonly test_cb_with_retry_grand_parent_path=$(dirname ${test_cb_with_retry_parent_path})
...
run_provided_test() {
    ...
    if [ "${test_group_to_run}" = "CircuitBreakerWithRetry" ]; then
        . ${test_cb_with_retry_parent_path}/test.sh
    fi
}
