CREATE PROCEDURE CALL_TEST_IN_NUMERIC_TYPES(@id AS INT, @smallIntVal AS SMALLINT, @bigIntVal AS BIGINT, @tinyIntVal AS TINYINT,
@bitVal AS BIT, @intVal AS INT, @decimalVal AS DECIMAL(4,2), @numericVal AS NUMERIC(4,3), @moneyVal AS MONEY, @smallMoneyVal AS SMALLMONEY)
AS
BEGIN
    INSERT INTO SELECT_UPDATE_TEST_NUMERIC_TYPES VALUES(@id, @smallIntVal, @bigIntVal, @tinyIntVal, @bitVal, @intVal, @decimalVal, @numericVal, @moneyVal, @smallMoneyVal);
END;
/
CREATE PROCEDURE CALL_TEST_OUT_NUMERIC_TYPES(@id AS INT, @smallIntVal AS SMALLINT OUTPUT, @bigIntVal AS BIGINT OUTPUT, @tinyIntVal AS TINYINT OUTPUT,
@bitVal AS BIT OUTPUT, @intVal AS INT OUTPUT, @decimalVal AS DECIMAL(4,2) OUTPUT, @numericVal AS NUMERIC(4,3) OUTPUT, @moneyVal AS MONEY OUTPUT, @smallMoneyVal AS SMALLMONEY OUTPUT)
AS
BEGIN
    SELECT @bigIntVal = BIGINT_VAL FROM SELECT_UPDATE_TEST_NUMERIC_TYPES WHERE ID = @id;
    SELECT @smallIntVal = SMALLINT_VAL FROM SELECT_UPDATE_TEST_NUMERIC_TYPES WHERE ID = @id;
    SELECT @tinyIntVal = TINYINT_VAL FROM SELECT_UPDATE_TEST_NUMERIC_TYPES WHERE ID = @id;
    SELECT @bitVal = BIT_VAL FROM SELECT_UPDATE_TEST_NUMERIC_TYPES WHERE ID = @id;
    SELECT @intVal = INTEGER_VAL FROM SELECT_UPDATE_TEST_NUMERIC_TYPES WHERE ID = @id;
    SELECT @decimalVal = DECIMAL_VAL FROM SELECT_UPDATE_TEST_NUMERIC_TYPES WHERE ID = @id;
    SELECT @numericVal = NUMERIC_VAL FROM SELECT_UPDATE_TEST_NUMERIC_TYPES WHERE ID = @id;
    SELECT @moneyVal = MONEY_VAL FROM SELECT_UPDATE_TEST_NUMERIC_TYPES WHERE ID = @id;
    SELECT @smallMoneyVal = SMALLMONEY_VAL FROM SELECT_UPDATE_TEST_NUMERIC_TYPES WHERE ID = @id;
END;
/
CREATE PROCEDURE CALL_TEST_INOUT_NUMERIC_TYPES(@id AS INT, @smallIntVal AS SMALLINT OUTPUT, @bigIntVal AS BIGINT OUTPUT, @tinyIntVal AS TINYINT OUTPUT,
@bitVal AS BIT OUTPUT, @intVal AS INT OUTPUT, @decimalVal AS DECIMAL(4,2) OUTPUT, @numericVal AS NUMERIC(4,3) OUTPUT, @moneyVal AS MONEY OUTPUT, @smallMoneyVal AS SMALLMONEY OUTPUT)
AS
BEGIN
    INSERT INTO SELECT_UPDATE_TEST_NUMERIC_TYPES VALUES(@id, @smallIntVal, @bigIntVal, @tinyIntVal, @bitVal, @intVal, @decimalVal, @numericVal, @moneyVal, @smallMoneyVal);
